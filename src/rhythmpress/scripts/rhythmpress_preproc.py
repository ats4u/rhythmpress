...
- Delegates to:  rhythmpress preproc_<PREPROC> [fm-args] [dirname] [extra CLI args]
"""
...
__all__ = ["find_langid", "extract_preproc_and_args", "dispatch_preproc", "main", "main_cli"]
...
def parse_args(argv: List[str]) -> tuple[argparse.Namespace, List[str]]:
    ...
    p.add_argument(
        ...
        nargs="?",
        default=".",
        help="Directory containing master-[langid].qmd or .md (default: .)",
    )
    p.add_argument(
        "--verbose", "-v",
        action="store_true",
...
def find_langid(dirpath: Path) -> Tuple[str, Path]:
    ...
    if len(candidates) > 1:
        listed = ", ".join(x.name for x in candidates.values())
        die(2, f"Ambiguous: {listed}. Set $LANG_ID.")

    (lang, path), = candidates.items()
    return lang, path
...
def dispatch_preproc(dirname: Path, *, extra_args: List[str], verbose: bool) -> int:
    if not dirname.is_dir():
        die(2, f"Not a directory: {dirname}")

    lang, master_qmd = find_langid(dirname)
    preproc, fm_args = extract_preproc_and_args(master_qmd)

    env = os.environ.copy()
    env.setdefault("LANG_ID", lang)

    passthru = [str(dirname)] + extra_args
    delegate_argv = ["rhythmpress", f"preproc_{preproc}", *fm_args, *passthru]
...
def main(argv: List[str]) -> int:
    ...
    return dispatch_preproc(dirpath, extra_args=unknown, verbose=ns.verbose)
