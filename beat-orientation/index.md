...
* シラブル拍リズム言語  (**拍の順番→ 弱強**)
    * 母音は等間隔に発音する。
    * 末子音は出来るだけ頭子音としてまとめて発音する。(MOP)
* ストレス拍リズム言語 (**拍の順番→ 弱強**)
    * シラブル拍リズム言語の全ルールを踏襲しルール追加する。
    * アクセントのある母音が等間隔に並んでいる。
    * アクセントのない子音と母音は縮小してアクセントの等間隔性を守る。
    * 末音節は頭音節にできるだけまとめて発音する。(MPOP)
* モーラ拍言語 (**拍の順番→ 強弱) 注意！逆**
    * 母音ないしは子音が等間隔に並んでいる。
    * 末子音（多重子音）は新たにシラブルを作って必ずばらして発音する。(MiOP)

### 言語リズム投影理論 ( LRPT Linguistic Rhythm Projection Theory )＝頭子音構成軸が弱拍強拍の順序を決める

 **再帰的韻律同一性原理**  **RPEP (Recursive Prosodic Equivalence Principle)** は、頭子音構成軸(OSA) にも再帰的に適用が出来ます。更に言語・音楽を超えて、歩き方やスポーツ格闘などその他の行動原理にも反映します。
...
### 能動的事前分割主義 (Proactive Divisism )と受動的事後付け足し主義（Reactive Appndism )  
