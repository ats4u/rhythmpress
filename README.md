...
If you want a clean Quarto preview state (removes `./.site`, `./.site-*`, and `./.quarto` first):
...
* Page JavaScript is disabled by default for deterministic captures; use `--enable-javascript` and `--wait-ms` when dynamic embeds must render.
...
* `rhythmpress clean-start`
  Clean Quarto preview state (`.quarto/`, `.site/`, `.site-*/`) and then start preview.
